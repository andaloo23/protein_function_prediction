"""
Main training script for ESM model.
"""
...
from src.models import ESMModel
from src.training import Trainer
from src.evaluation import Evaluator
from src.config import create_config, ESMConfig


def main():
    parser = argparse.ArgumentParser(description="Train ESM model for protein function prediction")
    ...
    parser.add_argument("--freeze-esm", action="store_true", default=True,
                       help="Freeze ESM parameters")
    parser.add_argument("--unfreeze-esm", action="store_false", dest="freeze_esm",
                       help="Unfreeze ESM parameters")
    parser.add_argument("--hidden-dims", type=int, nargs="+", default=[320, 200, 128],
    ...
    if args.config_file:
        ...
        config = create_config("esm", args.size, **{k: v for k, v in vars(args).items() if v is not None})
    ...
    data_processor.setup_labels()  # ESM doesn't need vocabulary
    ...
    train_dataset, val_dataset = data_processor.create_esm_datasets(
    ...
    if args.eval_only:
        ...
        results = evaluator.evaluate(model_type="esm", top_k=config.top_k)
